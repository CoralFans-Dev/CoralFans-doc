...
## 1. Install BDS and LeviLamina

The CoralFans plugin depends on the LeviLamina loader, and LeviLamina is based on the official BedrockDedicatedServer (BDS). So before using this plugin, you need to install BDS and LeviLamina.
...
+ **Enable loopback**
  + UWP apps have loopback disabled by default. You need to enable it to connect to a local server. **This step can be skipped for multiplayer games on cloud servers**. Open `powershell` as administrator and run the following command (from the official Microsoft website): `CheckNetIsolation.exe LoopbackExempt -a -p=S-1-15-2-1958404141-86561845-1752920682-3514627264-368642714-62675701-733520436`
...
2. Find the resource pack in the `assets` folder of the [prerequisite plugin](https://github.com/OEOTYAN/BedrockServerClientInterface), pack it, install it into your game, and enable it globally.
